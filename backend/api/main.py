...
async def create_reagent(
    ...
    result = await db.execute(
        select(Reagent).where(Reagent.reagent_id == reagent.reagent_id)
    )
    existing = result.scalar_one_or_none()
    if existing:
        raise HTTPException(400, f"试剂ID '{reagent.reagent_id}' 已存在")

    # 创建试剂目录
    reagent_dir = IMAGES_DIR / reagent.reagent_id
    reagent_dir.mkdir(parents=True, exist_ok=True)

    # 写入数据库
    db_reagent = Reagent(**reagent.dict())
    db.add(db_reagent)
    await db.commit()
    ...
    return {
        ...
        "message": f"试剂 {reagent.reagent_id} 创建成功，请上传图片完成注册",
    }
